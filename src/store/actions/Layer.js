...
const calculateRarity = (
  ...
  if (numberOfImagesWithFixedRarity > 0) {
    images = imagesForCalculate.map((imageMapped) => {
...
const updateFixRarity = (images, image, fixRarity) => {
  ...
  images = imagesForSettingFixRarity.map((imageMapped) => {
    if (imageMapped.imageId === image.imageId) {
      return {
        ...imageMapped,
