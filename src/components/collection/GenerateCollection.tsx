...
const GenerateCollection = (collection: any) => {
  ...
  const generateCollectionHandler = async () => {
    ...
    const generateCollectionResponse = await dispatch(generateCollection(collectionData)).unwrap()
  ...
  return (
    <button type='reset' className='btn btn-success mr-2' onClick={generateCollectionHandler}>
      GENERATE COLLECTION
    </button>
  )
}
