...
const GenerateCollection = () => {
  ...
  const receiptData = useSelector(
    (state) => state.collectionsStore.receiptData
  );
  const transactionStatus = useSelector(
    (state) => state.collectionsStore.transactionStatus
  );
  ...
  const [transactionStatusState, setTransactionStatusState] = useState(0);
    ...
    if (generateCollectionResponse.success) {
      ...
      evtSource.onmessage = function (e) {
        ...
        if (obj.iteration !== null) {
          ...
          const getGeneratedCollectionFunction = async (id) => {
            await wait(4000);
  ...
  const closePaymentFormModalHandler = (status) => {
    setShowPaymentFormModal(false);
  };
